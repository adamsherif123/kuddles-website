...
export function ProductDetail({ product }: { product: Product }) {
  ...
  const [openCareImage, setOpenCareImage] = useState<null | { src: string; label: string }>(null)
  ...
  // Update image when color changes to show matching color image
  useEffect(() => {
    const matchingImageIndex = safeImages.findIndex((img) => img.color === selectedColor)
    if (matchingImageIndex !== -1) {
      setSelectedImageIndex(matchingImageIndex)
    }
  }, [selectedColor, safeImages])
      ...
      <div className="container mx-auto px-4">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12">
          ...
          <div className="relative">
            ...
            <div className="relative aspect-[0.85] rounded-2xl overflow-hidden bg-white border mb-4">
              <Image
          ...
          <div>
            ...
            <Accordion type="single" collapsible className="w-full">
              <AccordionItem value="size-charts">
                <AccordionTrigger>
              ...
              <AccordionItem value="care">
                ...
                <AccordionContent>
                  <div className="grid grid-cols-2 gap-3 max-w-sm">
                    {[
                      { src: "/images/care-1.png", label: "Care Instructions 1" },
                      { src: "/images/care-2.png", label: "Care Instructions 2" },
                    ].map((item) => (
                      <button
                        key={item.src}
                        type="button"
                        onClick={() => setOpenCareImage(item)}
                        className="text-left rounded-2xl border bg-white p-2 hover:shadow-sm transition"
                        title="Click to expand"
                      >
                        <p className="text-[11px] font-medium text-muted-foreground mb-1">{item.label}</p>
                  ...
                  {/* Lightbox modal for care images */}
                  {openCareImage && (
                    <div
                      className="fixed inset-0 z-[999] bg-black/60 flex items-center justify-center p-4"
                      onClick={() => setOpenCareImage(null)}
                      role="dialog"
                      aria-modal="true"
                    >
                      <div
                      ...
                      >
                        ...
                        <p className="text-sm font-semibold mb-2 pr-10">{openCareImage.label}</p>

                        {/* No scrolling: fit to viewport */}
                        <div className="flex items-center justify-center">
                          <img
                            src={openCareImage.src}
                            alt={openCareImage.label}
                            className="h-[80vh] w-auto max-w-[92vw] object-contain"
                          />
                        </div>
                      </div>
